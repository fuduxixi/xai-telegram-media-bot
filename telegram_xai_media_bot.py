...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "常用命令\n"
        "1. 文生图：/image 提示词\n"
        "2. 快速 4 张图：/image4 提示词\n"
        "3. Pro 图片模型：/imagepro 提示词\n"
        "4. 文生视频：/video 提示词\n"
        "5. 图生视频：回复图片后发送 /img2video 提示词\n"
        "6. 查看当前配置：/models\n"
        "7. 查看详细帮助：/help\n"
        "8. 查看你的用户 ID：/me"
    )
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "使用说明\n\n"
        "图片命令\n"
        "1. 文生图：/image [-r 比例] [-n 数量] [-m 模型] 提示词\n"
        "2. 快速 4 张：/image4 提示词\n"
        "3. Pro 模型：/imagepro 提示词\n\n"
        "视频命令\n"
        "1. 文生视频：/video [-d 时长] [-r 比例] [-q 分辨率] [-n 数量] 提示词\n"
        "2. 图生视频：回复一张图片后发送 /img2video [-d 时长] [-r 比例] [-q 分辨率] [-n 数量] 提示词\n"
        "3. /video 支持回复文字或文本附件（txt / md / csv / tsv / json / yaml）使用\n\n"
        "当前范围\n"
        f"1. 图片模型：grok-imagine-image / grok-imagine-image-pro\n"
        f"2. 图片比例：auto / 16:9 / 9:16 / 1:1 / 3:2 / 2:3 / 4:3 / 3:4 / 4:5 / 5:4 / 2:1 / 1:2 / 19.5:9 / 9:19.5 / 20:9 / 9:20\n"
        f"3. 图片数量：1 到 {MAX_IMAGE_COUNT}\n"
        f"4. 视频比例：16:9 / 9:16 / 1:1 / 3:2 / 2:3\n"
        f"5. 视频分辨率：480p / 720p\n"
        f"6. 视频数量：1 到 {MAX_VIDEO_COUNT}\n"
        f"7. 文本附件大小限制：不超过 {MAX_TEXT_ATTACHMENT_BYTES // 1024}KB\n\n"
        "当前默认配置\n"
        f"1. 图片模型：{DEFAULT_IMAGE_MODEL}\n"
        f"2. 图片比例：{DEFAULT_IMAGE_RATIO}\n"
        f"3. 图片默认数量：{DEFAULT_IMAGE_COUNT}\n"
        f"4. 视频默认数量：{DEFAULT_VIDEO_COUNT}\n\n"
        "示例\n"
        "1. /image -r 1:1 一只在街机厅里的可爱机器人\n"
        "2. /image4 一只在街机厅里的可爱机器人\n"
        "3. /imagepro 黑色丝绸上的高端香水瓶\n"
        "4. /video -d 12 -r 9:16 -q 720p -n 2 霓虹雨夜中行走的赛博女孩\n"
        "5. 回复一段分镜文字后发送：/video -d 12 -r 16:9 -n 3\n"
        "6. 回复一张图片后发送：/img2video -d 8 -r 9:16 -n 2 眨眼并微笑"
    )
...
async def models(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "当前配置\n\n"
        "图片设置\n"
        f"1. 默认模型：{DEFAULT_IMAGE_MODEL}\n"
        f"2. 默认比例：{DEFAULT_IMAGE_RATIO}\n"
        f"3. 默认数量：{DEFAULT_IMAGE_COUNT}\n"
        f"4. 最大数量：{MAX_IMAGE_COUNT}\n\n"
        "视频设置\n"
        f"1. 默认数量：{DEFAULT_VIDEO_COUNT}\n"
        f"2. 最大数量：{MAX_VIDEO_COUNT}\n"
        f"3. 审核改写模式：{VIDEO_REWRITE_MODE}\n\n"
        "支持的图片模型\n"
        "1. grok-imagine-image\n"
        "2. grok-imagine-image-pro"
    )
